const { getAvailableLiquidity } = require('../main'); // adjust the path as needed
jest.mock('./manager'); // adjust the path as needed

describe('getAvailableLiquidity', () => {
    it('should calculate liquidity correctly', async () => {
        const owner = "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266"
        const token0 = "0x5FbDB2315678afecb367f032d93F642f64180aa3"
        const token1 = "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512"
        const fee = 3000;
        const liquidity = await getAvailableLiquidity(token0, token1, fee);
        expect(liquidity).toBe('ExpectedLiquidityValue1');
    
      });
});